...
export function PageLoader({ 
  isLoading, 
  onLoadingComplete, 
  minDuration = 800 // Default minimum duration
}: PageLoaderProps) {
  ...
  const startLoading = useCallback(() => {
    ...
    // Smooth progress animation from 0 to 85%
    progressIntervalRef.current = setInterval(() => {
      setProgress(current => {
        ...
        // Logarithmic progression for smooth deceleration
        const increment = Math.max(1, 15 * Math.exp(-current / 20));
        return Math.min(85, current + increment);
      });
    }, 50);
  }, [cleanupTimers]);
  ...
  }, [isLoading, isVisible, startLoading, completeLoading, cleanupTimers]);
